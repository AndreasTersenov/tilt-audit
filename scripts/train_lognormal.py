...
from tilt_audit import lognormal, misspec
...
def main():
    ...
    p.add_argument("--eps", type=float, default=0.0,
                   help="spectral-slope contamination of the g prior")
    ...
    print(f"UNet params: {n_params/1e6:.2f}M, eps={args.eps}, lam={lam:.4f}",
          flush=True)
    ...
    def train_step(params, opt_state, ema, key):
        ...
        x0 = lognormal.kappa(unpack(z, basis), lam)  # THE changed line
        ...
        if time.time() - t_last_ckpt > 1800:
            with out.open("wb") as f:
                pickle.dump({"params": params, "ema": ema, "eps": args.eps,
                             "lam": lam, "n": args.n, "step": step,
                             "losses": losses}, f)
            t_last_ckpt = time.time()
    with out.open("wb") as f:
        pickle.dump({"params": params, "ema": ema, "eps": args.eps,
                     "lam": lam, "n": args.n, "step": args.steps,
                     "losses": losses}, f)
